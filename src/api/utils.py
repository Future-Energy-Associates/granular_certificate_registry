...
def sqlmodel_obj_to_json(
    ...
    if sqlmodel_obj is None:
        ...
        json_content = [
            ...
            else json.loads(parse_nans_to_null(response_model.from_orm(elem).json(), replace_nan))
            for elem
            in sqlmodel_obj
        ]
    elif response_model is not None:
        json_content = json.loads(parse_nans_to_null(response_model.from_orm(sqlmodel_obj).json(), replace_nan))
    else:
...
def format_json_response(
    ...
    if send_raw:
        if pagination_metadata is not None:
            json_content = {'content': sqlmodel_obj, 'pagination_metadata': pagination_metadata}
        else:
    ...
    if pagination_metadata is not None:
        json_content = {'content': json_content, 'pagination_metadata': pagination_metadata}
...
def construct_pagination_metadata(
    ...
    for param in params:
        ...
        if value not in null_values:
            query = query.filter(func.lower(col(getattr(table_schema, param))).contains(value.strip().lower()))
